...
compare_thread = threading.Thread(target=hey())
...
class Application(Frame):
    ...
    def Compare(self):
        ...
        if os.path.isfile(self.file1) is True and os.path.isfile(self.file2) is True:
            ...
            if self.file3 == 'Default: same folder of the running program' and self.file4 == 'Default: reportH2.html':
                #If open file when completed is selected, open the html report using web browser
                if CheckVar2.get() == 1:
    ...
    def createWidgets(self):
        ...
        self.Check_C = Checkbutton(self, text = "Generate the Overall report separately      ", variable = CheckVar3,onvalue = 1, offvalue = 0, width = 40)
        self.Check_C.grid(row = 17, column = 1, columnspan = 1)
        self.Check_D = Checkbutton(self, text = "Create exception traceback log                  ", variable = CheckVar4,onvalue = 1, offvalue = 0, width = 40)
        self.Check_D.grid(row = 18, column = 1, columnspan = 1)
    ...
    def compare(self):
            ...
            for current_file_index in range(number_of_files):
                ...
                try:
                    ...
                    #current_file_A_original = [i for i in current_file_A]
    ...
    def writeEnding(self):
        ...
        if len(self.file_in_A_not_in_B) != 0 or len(self.file_in_B_not_in_A) != 0:
            ...
            for i in self.file_in_A_not_in_B:
                f.write('<p3 style="background-color: ' + self.background_color + '">   File type ' + i + ' is in folder A but not in folder B</p3>' + '\n')
            for i in self.file_in_B_not_in_A:
                f.write('<p3 style="background-color: ' + self.background_color + '">   File type ' + i + ' is in folder B but not in folder A</p3>' + '\n')
            f.write('<pre>' + '\n')
            for i in range(len(self.file_name_list)):
        ...
        for file1 in files_in_folder1:
            ...
            for file2 in files_in_folder2:
                if file1.split(".")[1] == file2.split('.')[1]:
                    ...
                    break
            if found == False:
                self.file_in_A_not_in_B.append(file1.split('.')[1])
        ...
        for file2 in files_in_folder2:
            ...
            for file1 in files_in_folder1:
                if file2.split(".")[1] == file1.split('.')[1]:
                    found = True
            if found == False:
                self.file_in_B_not_in_A.append(file2.split('.')[1])
